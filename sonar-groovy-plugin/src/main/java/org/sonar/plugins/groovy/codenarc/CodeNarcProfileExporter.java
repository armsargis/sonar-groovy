...
public class CodeNarcProfileExporter {
    ...
    public void exportProfile(ActiveRules activeRules) {
        try {
            Collection<org.sonar.api.batch.rule.ActiveRule> byRepository = activeRules.findByRepository(CodeNarcRulesDefinition.REPOSITORY_KEY);
            generateXML(byRepository);
        } catch (IOException e) {
